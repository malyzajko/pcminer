var pcminer = (function () {
  ...
  function pc(x, conf) {
    if (conf == undefined) {
      ...
      for (var i = 0; i < x.committees.length; i++) {
        var committee = x.committees[i];
        if (
          committee.conference.series == conf &&
          committee.role.indexOf("PC Member") != -1 &&
          withinRange(committee)
        ) {
          count++;
        }
      }
  ...
  function chair(x, conf) {
    for (var i = 0; i < x.committees.length; i++) {
      ...
      if (
        committee.conference.series == conf &&
        x.committees[i].role.indexOf("Chair") != -1
      ) {
        return true;
      }
    }
  ...
  function nameFilterChanged(val) {
    if (val != nameFilter) {
      ...
      if (
        ...
        currentAuthor.indexOf(nameFilter) != 0
      ) {
        currentAuthor = undefined;
  ...
  function updateNrEntries() {
    ...
    document.getElementById("nrpubs").innerHTML =
      '<span float="left">' + nrP + " publication records selected</span>";
    document.getElementById("nrcommittees").innerHTML =
          ...
          for (var j = 0; j < x.committees.length; j++) {
            clip.append(
              x.committees[j].conference.series +
                " " +
                x.committees[j].conference.year +
                " "
            );
          }
      ...
      for (var i = 0; i < list.length; i++) {
        if (mapCharacters(list[i].author) == authorName) {
          ...
          arr.sort(function (a, b) {
            return a.conference.year - b.conference.year;
          });
            ...
            for (var j = 0; j < arr.length; j++) {
              ...
              if (conferences[pub.conference.series] && withinRange(pub)) {
                ...
                text =
                  text +
                  "&nbsp;&nbsp;&nbsp;&nbsp;" +
                  getPublication(arr[j], authorName) +
                  "<br>";
              }
            }
          }
          ...
          if (list[i].committees.length > 0) {
            ...
            for (var j = 0; j < arr.length; j++) {
              var committee = arr[j];
              if (
                conferences[committee.conference.series] &&
                withinRange(committee)
              ) {
                if (!committeePrinted) {
  ...
  function getPublication(p, authorName) {
    ...
    return (
      ...
      color +
      '">' +
      getAuthors(p.authors, authorName) +
      ": " +
      p.title +
      ", " +
      getConference(p.conference) +
      " (session = " +
      p.session +
      "),  pp." +
      p.pages +
  ...
  function getAuthors(as, authorName) {
    ...
    for (var i = 0; i < as.length; i++) {
      if (mapCharacters(as[i]) == authorName) {
        text =
          text +
          "<b><font onMouseOver='pcminer.mouseOverAuthor(this)' " +
          "               onMouseOut='pcminer.mouseOutAuthor(this)' " +
          "               onclick='pcminer.authorClicked(\"" +
          mapCharacters(authorName) +
          "\")'>" +
          mapCharacters(authorName) +
          "</font></b>";
      } else {
        text =
          text +
          "<font onMouseOver='pcminer.mouseOverAuthor(this)' " +
          "            onMouseOut='pcminer.mouseOutAuthor(this)' " +
          "            onclick='pcminer.authorClicked(\"" +
          mapCharacters(as[i]) +
          "\")'>" +
  ...
  function installCheckboxes() {
    ...
    form +=
      '<label><input type="checkbox" id="all-technical" checked onclick="pcminer.confsSelected(\'all-technical\'' +
      techTrackNames +
      ')" /> <b>Technical Tracks</b></label><br />' +
      technicalTracks +
      "<hr />";
    form +=
      '<label><input type="checkbox" id="all-artifacts" onclick="pcminer.confsSelected(\'all-artifacts\'' +
      artifactTrackNames +
      ')" /> <b>Artifact Tracks</b></label><br />' +
      artifactTracks +
      "<br />";
